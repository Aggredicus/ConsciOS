...
function providerFromHost(host){
  ...
  return {
    kind:'browser-transformers-local',
    name:`${provenance.modelId} via ${provenance.runtime}`,
    hiddenState:'none',
    ...
    inferenceLocation:provenance.inferenceLocation,
    remoteInference:provenance.remoteInference
  };
}
...
export class BrowserTransformersCognitiveModel {
  ...
  async infer(input){
    ...
    const provider=providerFromHost(this.host);
    try{
      const result=await this.host.generate({
        userText:buildTaskInstruction(input),
        contextManifest:input.contextManifest,
        maxNewTokens:input.maxResponseUnits,
        doSample:false
      });
      ...
      return assertValidModelOutput({
        ...
        content:cancelled?null:{
          ...
          accessibleArtifactIds:[...result.contextArtifactIds],
          text:result.text,
          confidenceBasis:'uncalibrated-generative-output'
        },
        confidence:cancelled?0:null,
        causalSourceIds:[...input.causalSourceIds],
        timing:{
          elapsedMs:result.telemetry?.elapsedMs??0,
          ttftMs:result.telemetry?.ttftMs??null,
          streamed:Boolean(result.telemetry?.streamed)
        },
        failure:cancelled?'cancelled by caller':null,
        epistemicStatus:cancelled?'error':input.expectedEpistemicStatus
      });
    }catch(error){
      return assertValidModelOutput({
        requestId:input.requestId,
        ...
        confidence:0,
        causalSourceIds:[...input.causalSourceIds],
        timing:{elapsedMs:0,ttftMs:null,streamed:false},
