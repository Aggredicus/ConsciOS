export function expressV0({ guardian, guardianEvent, workspace, meta }, { makeEvent, pushEvent }) {
  if (guardian.decision!=='allow') return null;
  ...
  return pushEvent(makeEvent({id:'expression-1',source:'Expression',target:'Human',type:'expression.report',content:text,confidence:meta.confidence,salience:.7,causalParents:[guardianEvent.id,...workspace.map(w=>w.id)],epistemicStatus:'expression',globallyAvailable:true}));
}
