...
import { comparePolicies } from '../experiments/workspace-policies.mjs';
...
assert.deepEqual(
  result.raw.metrics.roots,
  ['obs-user', 'obs-user'],
  'raw top-k baseline should preserve v0 duplicate-root saturation'
);
assert.deepEqual(
  result.diverse.metrics.roots,
  ['obs-user', 'obs-runtime'],
  'hard source diversity should represent two distinct root observations'
);
assert.deepEqual(
  result.soft.metrics.roots,
  ['obs-user', 'obs-runtime'],
  'soft penalty 0.25 should prefer the runtime observation over a repeated user root'
);
...
console.log(JSON.stringify({
  ...
  diverse: result.diverse.metrics,
